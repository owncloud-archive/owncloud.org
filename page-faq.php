...
<p>Note that ownCloud is NOT a backup tool: the desktop client, for example, will synchronize the contents of a local folder with the server. That works for file modifications and additions, but also deletions. If you delete a file on the server, it gets deleted on the desktop you installed the client on! ownCloud does have a <strong>trasn</strong> and <strong>Versions</strong> support, which means deleted or modified files can be brought back, but this can be disabled and depending on the space usage settings, these older files do get deleted over time. You should always backup your ownCloud server!</p>
<p>Similarly, note that External Storage and the Server to Server sharing which ownCloud supports, connects ownCloud to an external storage like FTP or Samba, or to another ownCloud server. Files will show up in your web interface and can be synced with the client. However, they are <strong>not replicated</strong> on your ownCloud server! They are merely fetched whenever you request them in the web interface. Once the external storage disappears or other ownCloud server retracts your access, you can no longer access the files! Make a copy if you want to have them on your own ownCloud server.</p>
...
<ul>
	<li>ownCloud is being used by many large corporations in security sensitive situations (including banks and defense agencies), see <a href="#entcustomers">ownCloud, Inc. customers</a> and has undergone independent security audits. As an open source project, it benefits from a <a href="#communitysize">large number of eyes</a> looking at its code.</li>
